...
export const AppBar = () => {
  ...
  const [userLogoModalIsOpen, setUserLogoModalIsOpen] = React.useState(false);
  const [userInfoModalIsOpen, setUserInfoModalIsOpen] = React.useState(false);
  const { toggleUserLogoModal, userLogoModalVisible } = useModal();
  const { toggleUserInfoModal, userInfoModalVisible } = useModal();

  const toggleModal = () => {
    setOpen(!open);
  };

  function openUserLogoModal() {
    setUserLogoModalIsOpen(true);
  }

  function closeUserLogoModal() {
    setUserLogoModalIsOpen(false);
  }
      ...
      >
        ...
        <Box display="flex" alignItems="center">
          <Box
            ...
            alignItems="center"
            mr={{ xs: 24, md: 50 }}
            onClick={toggleUserLogoModal}
          >
            <Box>
              <UserLogoModal
                userLogoModalVisible={userLogoModalVisible}
                toggleUserLogoModal={toggleUserLogoModal}
